...
upload(){
  ...
  # Extract the substring after the version
  suffix="${filename#*${version}-}"

  # Check if the suffix is not empty and contains any characters
  if [[ -n "$suffix" ]]; then
    mvn_options+=" -Dclassifier=$suffix"
  fi
...
#file_paths_jar=($(find "$directory" -type f -name "*.war"))
