...
function download_artifacts(){ 
     local go_for_download=false
     
     if [ -f "$2" ]; then
       #echo "File Found"
       if grep -q "$downloadUrl" "$2"; then
         go_for_download=true
          sed -i "\#$downloadUrl#d" "$2"         
       else 
         echo "Not Found file:$downloadUrl"
       fi
     fi
    
     if [ "$go_for_download" = true ]; then
