...
file_path="/home/premy/scripts/artifacts2.txt"
# Function to wait for the deployment to finish in Nexus
wait_for_deploy() {
    local repo_url="<ur>/repository"
    local repository="$1"
    local group_id="$2"
    local artifact_id="$3"
    local version="$4"
...
#find "$directory" -type f -name "*.jar" -exec echo {} \;
...
upload(){
  ...
  mvn_options="-DgroupId=$trimmedGroupId -DartifactId=$artifactId -Dversion=$version -Dpackaging=$extension -DrepositoryId=$repositoryId -Durl=$url -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true"
